...
def show_loading_screen():
    ...
    Label(load, text="Activating Smart Surveillance...",
    ...
    try:
        ...
        print("Error loading eye GIF:", e)
        Label(load, text="👁", font=("Arial", 80), bg="#0f172a", fg="white").pack(pady=20)

    load.after(4000, lambda: [load.destroy(), open_main_dashboard()])
    load.mainloop()
...
def open_main_dashboard():
    ...
    Label(dash, text="🎥 Smart Surveillance System",
          font=("Arial", 28, "bold"), fg="white", bg="#0f172a").pack(pady=40)
    Label(dash, text="Camera feed and alerts will appear here.",
          font=("Arial", 14), fg="#cbd5e1", bg="#0f172a").pack(pady=10)
    Button(dash, text="Exit", command=dash.destroy,
           bg="#ef4444", fg="white", activebackground="#dc2626",
           font=("Arial", 14, "bold"), padx=25, pady=8, relief="flat").pack(pady=40)
...
def build_login_ui():
    ...
    if os.path.exists(bg_path):
        ...
        bg_image = Image.blend(bg_image, Image.new("RGB", (screen_w, screen_h), (6,10,25)), 0.35)
    else:
    ...
    draw.rounded_rectangle((0,0,plate_w,plate_h), radius=20, fill=(8,10,20,180), outline=(255,255,255,18), width=2)
    plate = plate.filter(ImageFilter.GaussianBlur(1))
    plate_photo = ImageTk.PhotoImage(plate)
    plate_label = Label(root, image=plate_photo, bd=0)
    plate_label.image = plate_photo
    plate_label.place(relx=0.5, rely=0.5, anchor=CENTER, width=plate_w, height=plate_h)

    # Login Frame
    frame = Frame(root, bg="#000000", bd=0)
    frame.place(relx=0.5, rely=0.5, anchor=CENTER, width=plate_w-20, height=plate_h-20)

    Label(frame, text="Smart Surveillance", font=("Arial", 20, "bold"), fg="white", bg="#000000").pack(pady=(12,6))
    Label(frame, text="Sign in to continue", font=("Arial", 10), fg="#cbd5e1", bg="#000000").pack(pady=(0,10))

    Label(frame, text="Username", fg="#cbd5e1", bg="#000000", anchor="w").pack(fill="x", padx=30)
    entry_username = Entry(frame, font=("Arial", 12), justify="left")
    ...
    Button(btn_frame, text="Login", font=("Arial", 12, "bold"),
           bg="#2563eb", fg="white", activebackground="#1d4ed8", relief="flat", padx=18, pady=8,
    ...
    Button(btn_frame, text="Create Account", font=("Arial", 12, "bold"),
           bg="#22c55e", fg="white", activebackground="#16a34a", relief="flat", padx=14, pady=8,
           command=lambda: show_create_account(root)).grid(row=0, column=1, padx=8)

    root._bg_photo = bg_photo
    root._plate_photo = plate_photo
    root.mainloop()
